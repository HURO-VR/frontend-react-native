...
public class AlgorithmRunner : MonoBehaviour {
    ...
    /// <summary>
    /// Randomizes the positions of all objects managed by spawners in the scene.
    /// This function can only be called when the algorithm is not running.
    /// </summary>
    /// <remarks>
    /// - Destroys all existing child objects of each spawner
    /// - Triggers a new spawn sequence for each spawner
    /// - Sets the restart flag to false after completion
    /// - Will not execute if the algorithm is currently running
    /// </remarks>
